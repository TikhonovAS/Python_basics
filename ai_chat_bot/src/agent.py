...
class AI_Agent:
    ...
    def get_response(self, user_text):
        ...
        """
        # 1. Запоминаем, что спросил человек
        ...
        if "привет" in user_text.lower():
            answer = f"Привет! Рад видеть тебя снова. Мы уже обсудили {len(self.history)} тем."
        elif "как дела" in user_text.lower():
            answer = f"У меня все отлично, спасибо! Готов помогать с кодом."
        elif "кто ты" in user_text.lower():
            answer = f"Я {self.name}, твой персональный ИИ-асситент."
        else:
            answers = [
